<!DOCTYPE html>
<html lang="pt-br">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>R&R Reserva De Restaurante</title>


    <meta http-equiv="cache-control" content="max-age=0" />
    <meta http-equiv="cache-control" content="no-cache" />
    <meta http-equiv="expires" content="0" />
    <meta http-equiv="expires" content="Tue, 01 Jan 1980 1:00:00 GMT" />
    <meta http-equiv="pragma" content="no-cache" />

    <script src="/pages/home/home.js" defer></script>

    <link rel="stylesheet" href="/pages/home/home.css">
    <link rel="stylesheet" href="/geral/geral.css">
</head>
<body>
    <header class="cabecalho">
        <nav class="navegacao">
            <figure class="logo">
                <img src="/assets/logo.png" alt="">
            </figure>
            <div class="botoes_nav">
                <button type="button" onclick="scrollParaSecao('secao2')">Restaurantes</button>
                <button type="button" onclick="scrollParaSecao('secao3')"> Clientes</button>
            </div>
            <div class="btn_entrar">
                <button type="button" onclick="location.href='/pages/login-register/login.html'">Entrar</button>
            </div>
        </nav>
    </header>
    <main>
        <section class="principal" id="secao1">

            <div class="informacao_principal">
                <h1 class="titulo_principal_reserva">
                    Faça a sua reserva em seus restaurantes favoritos em um único lugar.
                </h1>
                <p class="paragrafo_principal">
                    O R&R oferece recursos adicionais, como avaliações detalhadas, menus atualizados e até mesmo sugestões personalizadas com base no histórico de preferências dos usuários. Com essa ferramenta, desfrutar de uma refeição memorável nos restaurantes favoritos nunca foi tão fácil e conveniente.
                </p>
            </div>

            <figure class="img_celular">

                <img src="../../assets/celular.png" alt="">

            </figure>

        </section>

        <section class="tela_resturantes" id="secao2">
                <nav class="bloco_navegacao">
                    <div class="bloco_pesquisa">
                        <input type="text" placeholder="Encontre seu restaurante...">
                        <button>
                            <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 13 13" fill="none">
                                <path d="M6.38462 10.8662L11 6.4331L6.38462 1.99998M10.359 6.4331L1 6.4331" stroke="white" stroke-width="2" stroke-miterlimit="10" stroke-linecap="square"/>
                            </svg>
                        </button>
                    </div>
                </nav>

                <ul class="lista_restaurantes">
                    <li class="bloco_conteudo">
                        <div class="bloco_informacao">
                           <div class="topo">
                                <h3 id="restaurant_Name">OUTBACK</h3>
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                                    <g clip-path="url(#clip0_15_167)">
                                    <path d="M11.625 23.25C9.32583 23.25 7.07821 22.5683 5.1665 21.2908C3.25478 20.0135 1.76478 18.1979 0.884906 16.0738C0.00503572 13.9495 -0.225178 11.6121 0.223379 9.35709C0.671923 7.10205 1.7791 5.03068 3.40488 3.40488C5.03068 1.7791 7.10205 0.671923 9.35709 0.223379C11.6121 -0.225178 13.9495 0.00503572 16.0738 0.884906C18.1979 1.76478 20.0135 3.25478 21.2908 5.1665C22.5683 7.07821 23.25 9.32583 23.25 11.625C23.25 14.7082 22.0252 17.665 19.8452 19.8452C17.665 22.0252 14.7082 23.25 11.625 23.25ZM11.625 1.9375C9.70894 1.9375 7.83602 2.50566 6.24292 3.57014C4.64982 4.63461 3.40814 6.14759 2.67492 7.91776C1.9417 9.68789 1.74986 11.6357 2.12364 13.515C2.49744 15.3941 3.42009 17.1203 4.7749 18.4751C6.12973 19.8299 7.85588 20.7526 9.73503 21.1264C11.6143 21.5002 13.5621 21.3084 15.3322 20.5751C17.1024 19.8418 18.6154 18.6001 19.6798 17.0071C20.7443 15.414 21.3125 13.5411 21.3125 11.625C21.3125 9.05574 20.2918 6.59167 18.4751 4.7749C16.6584 2.95815 14.1943 1.9375 11.625 1.9375Z" fill="white" fill-opacity="0.5"/>
                                    <path d="M11.5 12.7917C11.2441 12.7883 10.9996 12.6853 10.8186 12.5043C10.6377 12.3233 10.5346 12.0788 10.5312 11.823V7.30212C10.5312 7.0452 10.6333 6.79879 10.815 6.61711C10.9966 6.43544 11.2431 6.33337 11.5 6.33337C11.7569 6.33337 12.0034 6.43544 12.185 6.61711C12.3667 6.79879 12.4688 7.0452 12.4688 7.30212V11.823C12.4654 12.0788 12.3623 12.3233 12.1814 12.5043C12.0004 12.6853 11.7559 12.7883 11.5 12.7917Z" fill="white" fill-opacity="0.5"/>
                                    <path d="M11.5 16.6667C11.2441 16.6633 10.9996 16.5603 10.8186 16.3793C10.6377 16.1983 10.5346 15.9538 10.5312 15.698V15.0521C10.5312 14.7952 10.6333 14.5488 10.815 14.3672C10.9966 14.1854 11.2431 14.0834 11.5 14.0834C11.7569 14.0834 12.0034 14.1854 12.185 14.3672C12.3667 14.5488 12.4688 14.7952 12.4688 15.0521V15.698C12.4654 15.9538 12.3623 16.1983 12.1814 16.3793C12.0004 16.5603 11.7559 16.6633 11.5 16.6667Z" fill="white" fill-opacity="0.5"/>
                                    </g>
                                    <defs>
                                    <clipPath id="clip0_15_167">
                                        <rect width="24" height="24" fill="white"/>
                                    </clipPath>
                                    </defs>
                                </svg>
                            </div>
                            <div class="bloco_inf">
                                <svg xmlns="http://www.w3.org/2000/svg" width="21" height="21" viewBox="0 0 21 21" fill="none">
                                    <path d="M13.125 18.375L7.875 16.5375L3.80625 18.1125C3.51458 18.2292 3.24479 18.1965 2.99687 18.0145C2.74896 17.8325 2.625 17.5881 2.625 17.2812V5.03125C2.625 4.84167 2.67983 4.67396 2.7895 4.52813C2.89917 4.38229 3.0485 4.27292 3.2375 4.2L7.875 2.625L13.125 4.4625L17.1937 2.8875C17.4854 2.77083 17.7552 2.80379 18.0031 2.98638C18.251 3.16896 18.375 3.41308 18.375 3.71875V15.9688C18.375 16.1583 18.3205 16.326 18.2114 16.4719C18.1023 16.6177 17.9527 16.7271 17.7625 16.8L13.125 18.375ZM12.25 16.2312V5.99375L8.75 4.76875V15.0062L12.25 16.2312Z" fill="white" fill-opacity="0.5"/>
                                </svg>
                                <span class="endereço">Av. Robert Kennedy 3999, São Paulo, SP, 04772-004</span>
                            </div>
                            <div class="tefone">
                                <svg xmlns="http://www.w3.org/2000/svg" width="23" height="23" viewBox="0 0 23 23" fill="none">
                                    <path d="M15.8663 12.3684L15.4298 12.8023C15.4298 12.8023 14.3923 13.834 11.5605 11.0181C8.72861 8.20234 9.76608 7.17075 9.76608 7.17075L10.0409 6.89745C10.7181 6.22418 10.7819 5.14324 10.1911 4.35412L8.9827 2.73991C8.25151 1.76321 6.83862 1.63419 6.00055 2.4675L4.49635 3.96316C4.0808 4.37636 3.80232 4.91198 3.8361 5.50617C3.92249 7.02628 4.61026 10.297 8.44805 14.1129C12.5178 18.1596 16.3365 18.3204 17.8981 18.1748C18.392 18.1288 18.8216 17.8773 19.1677 17.533L20.5291 16.1795C21.4481 15.2657 21.1889 13.6992 20.0132 13.0601L18.1823 12.0648C17.4102 11.6451 16.4697 11.7684 15.8663 12.3684Z" fill="white" fill-opacity="0.5"/>
                                </svg>
                                <span class="telefone">+55 (11) 97170-4067</span>
                            </div>
                            <div class="bloco_abaixo">
                                <div class="esquerda">
                                    <span class="aberto">Aberto</span>
                                    <div class="horario">
                                        <span class="horario"> - Fecha à(s) 23h</span>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="13" height="7" viewBox="0 0 13 7" fill="none">
                                            <g clip-path="url(#clip0_23_184)">
                                              <path d="M7.16912 -9.08414C7.16912 -9.47952 6.84813 -9.80005 6.45221 -9.80005C6.05628 -9.80005 5.73529 -9.47952 5.73529 -9.08414V-0.254595H0.716931C0.426974 -0.254595 0.165559 -0.0801992 0.0545904 0.18736C-0.0563684 0.454823 0.00496099 0.762759 0.209998 0.967509L5.9453 6.69478C6.0797 6.82909 6.26208 6.9045 6.45221 6.9045C6.64233 6.9045 6.82471 6.82909 6.95921 6.69478L12.6945 0.967509C12.8995 0.762759 12.9608 0.454823 12.8498 0.18736C12.7389 -0.0801992 12.4775 -0.254595 12.1875 -0.254595H7.16912V-9.08414Z" fill="white" fill-opacity="0.5"/>
                                            </g>
                                            <defs>
                                              <clipPath id="clip0_23_184">
                                                <rect width="13" height="7" fill="white"/>
                                              </clipPath>
                                            </defs>
                                        </svg>
                                    </div>
                                </div>
                                <button class="btn_reservar" onclick="userOnline()">Faça sua reserva</button>
                            </div>
                        </div>
                    </li>

                    <li class="bloco_conteudo">
                        <div class="bloco_informacao">
                            <div class="topo">
                                <h3>OUTBACK</h3>
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                                    <g clip-path="url(#clip0_15_167)">
                                        <path d="M11.625 23.25C9.32583 23.25 7.07821 22.5683 5.1665 21.2908C3.25478 20.0135 1.76478 18.1979 0.884906 16.0738C0.00503572 13.9495 -0.225178 11.6121 0.223379 9.35709C0.671923 7.10205 1.7791 5.03068 3.40488 3.40488C5.03068 1.7791 7.10205 0.671923 9.35709 0.223379C11.6121 -0.225178 13.9495 0.00503572 16.0738 0.884906C18.1979 1.76478 20.0135 3.25478 21.2908 5.1665C22.5683 7.07821 23.25 9.32583 23.25 11.625C23.25 14.7082 22.0252 17.665 19.8452 19.8452C17.665 22.0252 14.7082 23.25 11.625 23.25ZM11.625 1.9375C9.70894 1.9375 7.83602 2.50566 6.24292 3.57014C4.64982 4.63461 3.40814 6.14759 2.67492 7.91776C1.9417 9.68789 1.74986 11.6357 2.12364 13.515C2.49744 15.3941 3.42009 17.1203 4.7749 18.4751C6.12973 19.8299 7.85588 20.7526 9.73503 21.1264C11.6143 21.5002 13.5621 21.3084 15.3322 20.5751C17.1024 19.8418 18.6154 18.6001 19.6798 17.0071C20.7443 15.414 21.3125 13.5411 21.3125 11.625C21.3125 9.05574 20.2918 6.59167 18.4751 4.7749C16.6584 2.95815 14.1943 1.9375 11.625 1.9375Z" fill="white" fill-opacity="0.5"/>
                                        <path d="M11.5 12.7917C11.2441 12.7883 10.9996 12.6853 10.8186 12.5043C10.6377 12.3233 10.5346 12.0788 10.5312 11.823V7.30212C10.5312 7.0452 10.6333 6.79879 10.815 6.61711C10.9966 6.43544 11.2431 6.33337 11.5 6.33337C11.7569 6.33337 12.0034 6.43544 12.185 6.61711C12.3667 6.79879 12.4688 7.0452 12.4688 7.30212V11.823C12.4654 12.0788 12.3623 12.3233 12.1814 12.5043C12.0004 12.6853 11.7559 12.7883 11.5 12.7917Z" fill="white" fill-opacity="0.5"/>
                                        <path d="M11.5 16.6667C11.2441 16.6633 10.9996 16.5603 10.8186 16.3793C10.6377 16.1983 10.5346 15.9538 10.5312 15.698V15.0521C10.5312 14.7952 10.6333 14.5488 10.815 14.3672C10.9966 14.1854 11.2431 14.0834 11.5 14.0834C11.7569 14.0834 12.0034 14.1854 12.185 14.3672C12.3667 14.5488 12.4688 14.7952 12.4688 15.0521V15.698C12.4654 15.9538 12.3623 16.1983 12.1814 16.3793C12.0004 16.5603 11.7559 16.6633 11.5 16.6667Z" fill="white" fill-opacity="0.5"/>
                                    </g>
                                    <defs>
                                        <clipPath id="clip0_15_167">
                                            <rect width="24" height="24" fill="white"/>
                                        </clipPath>
                                    </defs>
                                </svg>
                            </div>
                            <div class="bloco_inf">
                                <svg xmlns="http://www.w3.org/2000/svg" width="21" height="21" viewBox="0 0 21 21" fill="none">
                                    <path d="M13.125 18.375L7.875 16.5375L3.80625 18.1125C3.51458 18.2292 3.24479 18.1965 2.99687 18.0145C2.74896 17.8325 2.625 17.5881 2.625 17.2812V5.03125C2.625 4.84167 2.67983 4.67396 2.7895 4.52813C2.89917 4.38229 3.0485 4.27292 3.2375 4.2L7.875 2.625L13.125 4.4625L17.1937 2.8875C17.4854 2.77083 17.7552 2.80379 18.0031 2.98638C18.251 3.16896 18.375 3.41308 18.375 3.71875V15.9688C18.375 16.1583 18.3205 16.326 18.2114 16.4719C18.1023 16.6177 17.9527 16.7271 17.7625 16.8L13.125 18.375ZM12.25 16.2312V5.99375L8.75 4.76875V15.0062L12.25 16.2312Z" fill="white" fill-opacity="0.5"/>
                                </svg>
                                <span class="endereço">Av. Robert Kennedy 3999, São Paulo, SP, 04772-004</span>
                            </div>
                            <div class="tefone">
                                <svg xmlns="http://www.w3.org/2000/svg" width="23" height="23" viewBox="0 0 23 23" fill="none">
                                    <path d="M15.8663 12.3684L15.4298 12.8023C15.4298 12.8023 14.3923 13.834 11.5605 11.0181C8.72861 8.20234 9.76608 7.17075 9.76608 7.17075L10.0409 6.89745C10.7181 6.22418 10.7819 5.14324 10.1911 4.35412L8.9827 2.73991C8.25151 1.76321 6.83862 1.63419 6.00055 2.4675L4.49635 3.96316C4.0808 4.37636 3.80232 4.91198 3.8361 5.50617C3.92249 7.02628 4.61026 10.297 8.44805 14.1129C12.5178 18.1596 16.3365 18.3204 17.8981 18.1748C18.392 18.1288 18.8216 17.8773 19.1677 17.533L20.5291 16.1795C21.4481 15.2657 21.1889 13.6992 20.0132 13.0601L18.1823 12.0648C17.4102 11.6451 16.4697 11.7684 15.8663 12.3684Z" fill="white" fill-opacity="0.5"/>
                                </svg>
                                <span class="telefone">+55 (11) 97170-4067</span>
                            </div>
                            <div class="bloco_abaixo">
                                <div class="esquerda">
                                    <span class="aberto">Aberto</span>
                                    <div class="horario">
                                        <span class="horario"> - Fecha à(s) 23h</span>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="13" height="7" viewBox="0 0 13 7" fill="none">
                                            <g clip-path="url(#clip0_23_184)">
                                                <path d="M7.16912 -9.08414C7.16912 -9.47952 6.84813 -9.80005 6.45221 -9.80005C6.05628 -9.80005 5.73529 -9.47952 5.73529 -9.08414V-0.254595H0.716931C0.426974 -0.254595 0.165559 -0.0801992 0.0545904 0.18736C-0.0563684 0.454823 0.00496099 0.762759 0.209998 0.967509L5.9453 6.69478C6.0797 6.82909 6.26208 6.9045 6.45221 6.9045C6.64233 6.9045 6.82471 6.82909 6.95921 6.69478L12.6945 0.967509C12.8995 0.762759 12.9608 0.454823 12.8498 0.18736C12.7389 -0.0801992 12.4775 -0.254595 12.1875 -0.254595H7.16912V-9.08414Z" fill="white" fill-opacity="0.5"/>
                                            </g>
                                            <defs>
                                                <clipPath id="clip0_23_184">
                                                    <rect width="13" height="7" fill="white"/>
                                                </clipPath>
                                            </defs>
                                        </svg>
                                    </div>
                                </div>
                                <button class="btn_reservar">Faça sua reserva</button>
                            </div>
                        </div>
                    </li>

                    <li class="bloco_conteudo">
                        <div class="bloco_informacao">
                            <div class="topo">
                                <h3>OUTBACK</h3>
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                                    <g clip-path="url(#clip0_15_167)">
                                        <path d="M11.625 23.25C9.32583 23.25 7.07821 22.5683 5.1665 21.2908C3.25478 20.0135 1.76478 18.1979 0.884906 16.0738C0.00503572 13.9495 -0.225178 11.6121 0.223379 9.35709C0.671923 7.10205 1.7791 5.03068 3.40488 3.40488C5.03068 1.7791 7.10205 0.671923 9.35709 0.223379C11.6121 -0.225178 13.9495 0.00503572 16.0738 0.884906C18.1979 1.76478 20.0135 3.25478 21.2908 5.1665C22.5683 7.07821 23.25 9.32583 23.25 11.625C23.25 14.7082 22.0252 17.665 19.8452 19.8452C17.665 22.0252 14.7082 23.25 11.625 23.25ZM11.625 1.9375C9.70894 1.9375 7.83602 2.50566 6.24292 3.57014C4.64982 4.63461 3.40814 6.14759 2.67492 7.91776C1.9417 9.68789 1.74986 11.6357 2.12364 13.515C2.49744 15.3941 3.42009 17.1203 4.7749 18.4751C6.12973 19.8299 7.85588 20.7526 9.73503 21.1264C11.6143 21.5002 13.5621 21.3084 15.3322 20.5751C17.1024 19.8418 18.6154 18.6001 19.6798 17.0071C20.7443 15.414 21.3125 13.5411 21.3125 11.625C21.3125 9.05574 20.2918 6.59167 18.4751 4.7749C16.6584 2.95815 14.1943 1.9375 11.625 1.9375Z" fill="white" fill-opacity="0.5"/>
                                        <path d="M11.5 12.7917C11.2441 12.7883 10.9996 12.6853 10.8186 12.5043C10.6377 12.3233 10.5346 12.0788 10.5312 11.823V7.30212C10.5312 7.0452 10.6333 6.79879 10.815 6.61711C10.9966 6.43544 11.2431 6.33337 11.5 6.33337C11.7569 6.33337 12.0034 6.43544 12.185 6.61711C12.3667 6.79879 12.4688 7.0452 12.4688 7.30212V11.823C12.4654 12.0788 12.3623 12.3233 12.1814 12.5043C12.0004 12.6853 11.7559 12.7883 11.5 12.7917Z" fill="white" fill-opacity="0.5"/>
                                        <path d="M11.5 16.6667C11.2441 16.6633 10.9996 16.5603 10.8186 16.3793C10.6377 16.1983 10.5346 15.9538 10.5312 15.698V15.0521C10.5312 14.7952 10.6333 14.5488 10.815 14.3672C10.9966 14.1854 11.2431 14.0834 11.5 14.0834C11.7569 14.0834 12.0034 14.1854 12.185 14.3672C12.3667 14.5488 12.4688 14.7952 12.4688 15.0521V15.698C12.4654 15.9538 12.3623 16.1983 12.1814 16.3793C12.0004 16.5603 11.7559 16.6633 11.5 16.6667Z" fill="white" fill-opacity="0.5"/>
                                    </g>
                                    <defs>
                                        <clipPath id="clip0_15_167">
                                            <rect width="24" height="24" fill="white"/>
                                        </clipPath>
                                    </defs>
                                </svg>
                            </div>
                            <div class="bloco_inf">
                                <svg xmlns="http://www.w3.org/2000/svg" width="21" height="21" viewBox="0 0 21 21" fill="none">
                                    <path d="M13.125 18.375L7.875 16.5375L3.80625 18.1125C3.51458 18.2292 3.24479 18.1965 2.99687 18.0145C2.74896 17.8325 2.625 17.5881 2.625 17.2812V5.03125C2.625 4.84167 2.67983 4.67396 2.7895 4.52813C2.89917 4.38229 3.0485 4.27292 3.2375 4.2L7.875 2.625L13.125 4.4625L17.1937 2.8875C17.4854 2.77083 17.7552 2.80379 18.0031 2.98638C18.251 3.16896 18.375 3.41308 18.375 3.71875V15.9688C18.375 16.1583 18.3205 16.326 18.2114 16.4719C18.1023 16.6177 17.9527 16.7271 17.7625 16.8L13.125 18.375ZM12.25 16.2312V5.99375L8.75 4.76875V15.0062L12.25 16.2312Z" fill="white" fill-opacity="0.5"/>
                                </svg>
                                <span class="endereço">Av. Robert Kennedy 3999, São Paulo, SP, 04772-004</span>
                            </div>
                            <div class="tefone">
                                <svg xmlns="http://www.w3.org/2000/svg" width="23" height="23" viewBox="0 0 23 23" fill="none">
                                    <path d="M15.8663 12.3684L15.4298 12.8023C15.4298 12.8023 14.3923 13.834 11.5605 11.0181C8.72861 8.20234 9.76608 7.17075 9.76608 7.17075L10.0409 6.89745C10.7181 6.22418 10.7819 5.14324 10.1911 4.35412L8.9827 2.73991C8.25151 1.76321 6.83862 1.63419 6.00055 2.4675L4.49635 3.96316C4.0808 4.37636 3.80232 4.91198 3.8361 5.50617C3.92249 7.02628 4.61026 10.297 8.44805 14.1129C12.5178 18.1596 16.3365 18.3204 17.8981 18.1748C18.392 18.1288 18.8216 17.8773 19.1677 17.533L20.5291 16.1795C21.4481 15.2657 21.1889 13.6992 20.0132 13.0601L18.1823 12.0648C17.4102 11.6451 16.4697 11.7684 15.8663 12.3684Z" fill="white" fill-opacity="0.5"/>
                                </svg>
                                <span class="telefone">+55 (11) 97170-4067</span>
                            </div>
                            <div class="bloco_abaixo">
                                <div class="esquerda">
                                    <span class="aberto">Aberto</span>
                                    <div class="horario">
                                        <span class="horario"> - Fecha à(s) 23h</span>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="13" height="7" viewBox="0 0 13 7" fill="none">
                                            <g clip-path="url(#clip0_23_184)">
                                                <path d="M7.16912 -9.08414C7.16912 -9.47952 6.84813 -9.80005 6.45221 -9.80005C6.05628 -9.80005 5.73529 -9.47952 5.73529 -9.08414V-0.254595H0.716931C0.426974 -0.254595 0.165559 -0.0801992 0.0545904 0.18736C-0.0563684 0.454823 0.00496099 0.762759 0.209998 0.967509L5.9453 6.69478C6.0797 6.82909 6.26208 6.9045 6.45221 6.9045C6.64233 6.9045 6.82471 6.82909 6.95921 6.69478L12.6945 0.967509C12.8995 0.762759 12.9608 0.454823 12.8498 0.18736C12.7389 -0.0801992 12.4775 -0.254595 12.1875 -0.254595H7.16912V-9.08414Z" fill="white" fill-opacity="0.5"/>
                                            </g>
                                            <defs>
                                                <clipPath id="clip0_23_184">
                                                    <rect width="13" height="7" fill="white"/>
                                                </clipPath>
                                            </defs>
                                        </svg>
                                    </div>
                                </div>
                                <button class="btn_reservar">Faça sua reserva</button>
                            </div>
                        </div>
                    </li>

                    <li class="bloco_conteudo">
                        <div class="bloco_informacao">
                            <div class="topo">
                                <h3>OUTBACK</h3>
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                                    <g clip-path="url(#clip0_15_167)">
                                        <path d="M11.625 23.25C9.32583 23.25 7.07821 22.5683 5.1665 21.2908C3.25478 20.0135 1.76478 18.1979 0.884906 16.0738C0.00503572 13.9495 -0.225178 11.6121 0.223379 9.35709C0.671923 7.10205 1.7791 5.03068 3.40488 3.40488C5.03068 1.7791 7.10205 0.671923 9.35709 0.223379C11.6121 -0.225178 13.9495 0.00503572 16.0738 0.884906C18.1979 1.76478 20.0135 3.25478 21.2908 5.1665C22.5683 7.07821 23.25 9.32583 23.25 11.625C23.25 14.7082 22.0252 17.665 19.8452 19.8452C17.665 22.0252 14.7082 23.25 11.625 23.25ZM11.625 1.9375C9.70894 1.9375 7.83602 2.50566 6.24292 3.57014C4.64982 4.63461 3.40814 6.14759 2.67492 7.91776C1.9417 9.68789 1.74986 11.6357 2.12364 13.515C2.49744 15.3941 3.42009 17.1203 4.7749 18.4751C6.12973 19.8299 7.85588 20.7526 9.73503 21.1264C11.6143 21.5002 13.5621 21.3084 15.3322 20.5751C17.1024 19.8418 18.6154 18.6001 19.6798 17.0071C20.7443 15.414 21.3125 13.5411 21.3125 11.625C21.3125 9.05574 20.2918 6.59167 18.4751 4.7749C16.6584 2.95815 14.1943 1.9375 11.625 1.9375Z" fill="white" fill-opacity="0.5"/>
                                        <path d="M11.5 12.7917C11.2441 12.7883 10.9996 12.6853 10.8186 12.5043C10.6377 12.3233 10.5346 12.0788 10.5312 11.823V7.30212C10.5312 7.0452 10.6333 6.79879 10.815 6.61711C10.9966 6.43544 11.2431 6.33337 11.5 6.33337C11.7569 6.33337 12.0034 6.43544 12.185 6.61711C12.3667 6.79879 12.4688 7.0452 12.4688 7.30212V11.823C12.4654 12.0788 12.3623 12.3233 12.1814 12.5043C12.0004 12.6853 11.7559 12.7883 11.5 12.7917Z" fill="white" fill-opacity="0.5"/>
                                        <path d="M11.5 16.6667C11.2441 16.6633 10.9996 16.5603 10.8186 16.3793C10.6377 16.1983 10.5346 15.9538 10.5312 15.698V15.0521C10.5312 14.7952 10.6333 14.5488 10.815 14.3672C10.9966 14.1854 11.2431 14.0834 11.5 14.0834C11.7569 14.0834 12.0034 14.1854 12.185 14.3672C12.3667 14.5488 12.4688 14.7952 12.4688 15.0521V15.698C12.4654 15.9538 12.3623 16.1983 12.1814 16.3793C12.0004 16.5603 11.7559 16.6633 11.5 16.6667Z" fill="white" fill-opacity="0.5"/>
                                    </g>
                                    <defs>
                                        <clipPath id="clip0_15_167">
                                            <rect width="24" height="24" fill="white"/>
                                        </clipPath>
                                    </defs>
                                </svg>
                            </div>
                            <div class="bloco_inf">
                                <svg xmlns="http://www.w3.org/2000/svg" width="21" height="21" viewBox="0 0 21 21" fill="none">
                                    <path d="M13.125 18.375L7.875 16.5375L3.80625 18.1125C3.51458 18.2292 3.24479 18.1965 2.99687 18.0145C2.74896 17.8325 2.625 17.5881 2.625 17.2812V5.03125C2.625 4.84167 2.67983 4.67396 2.7895 4.52813C2.89917 4.38229 3.0485 4.27292 3.2375 4.2L7.875 2.625L13.125 4.4625L17.1937 2.8875C17.4854 2.77083 17.7552 2.80379 18.0031 2.98638C18.251 3.16896 18.375 3.41308 18.375 3.71875V15.9688C18.375 16.1583 18.3205 16.326 18.2114 16.4719C18.1023 16.6177 17.9527 16.7271 17.7625 16.8L13.125 18.375ZM12.25 16.2312V5.99375L8.75 4.76875V15.0062L12.25 16.2312Z" fill="white" fill-opacity="0.5"/>
                                </svg>
                                <span class="endereço">Av. Robert Kennedy 3999, São Paulo, SP, 04772-004</span>
                            </div>
                            <div class="tefone">
                                <svg xmlns="http://www.w3.org/2000/svg" width="23" height="23" viewBox="0 0 23 23" fill="none">
                                    <path d="M15.8663 12.3684L15.4298 12.8023C15.4298 12.8023 14.3923 13.834 11.5605 11.0181C8.72861 8.20234 9.76608 7.17075 9.76608 7.17075L10.0409 6.89745C10.7181 6.22418 10.7819 5.14324 10.1911 4.35412L8.9827 2.73991C8.25151 1.76321 6.83862 1.63419 6.00055 2.4675L4.49635 3.96316C4.0808 4.37636 3.80232 4.91198 3.8361 5.50617C3.92249 7.02628 4.61026 10.297 8.44805 14.1129C12.5178 18.1596 16.3365 18.3204 17.8981 18.1748C18.392 18.1288 18.8216 17.8773 19.1677 17.533L20.5291 16.1795C21.4481 15.2657 21.1889 13.6992 20.0132 13.0601L18.1823 12.0648C17.4102 11.6451 16.4697 11.7684 15.8663 12.3684Z" fill="white" fill-opacity="0.5"/>
                                </svg>
                                <span class="telefone">+55 (11) 97170-4067</span>
                            </div>
                            <div class="bloco_abaixo">
                                <div class="esquerda">
                                    <span class="aberto">Aberto</span>
                                    <div class="horario">
                                        <span class="horario"> - Fecha à(s) 23h</span>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="13" height="7" viewBox="0 0 13 7" fill="none">
                                            <g clip-path="url(#clip0_23_184)">
                                                <path d="M7.16912 -9.08414C7.16912 -9.47952 6.84813 -9.80005 6.45221 -9.80005C6.05628 -9.80005 5.73529 -9.47952 5.73529 -9.08414V-0.254595H0.716931C0.426974 -0.254595 0.165559 -0.0801992 0.0545904 0.18736C-0.0563684 0.454823 0.00496099 0.762759 0.209998 0.967509L5.9453 6.69478C6.0797 6.82909 6.26208 6.9045 6.45221 6.9045C6.64233 6.9045 6.82471 6.82909 6.95921 6.69478L12.6945 0.967509C12.8995 0.762759 12.9608 0.454823 12.8498 0.18736C12.7389 -0.0801992 12.4775 -0.254595 12.1875 -0.254595H7.16912V-9.08414Z" fill="white" fill-opacity="0.5"/>
                                            </g>
                                            <defs>
                                                <clipPath id="clip0_23_184">
                                                    <rect width="13" height="7" fill="white"/>
                                                </clipPath>
                                            </defs>
                                        </svg>
                                    </div>
                                </div>
                                <button class="btn_reservar">Faça sua reserva</button>
                            </div>
                        </div>
                    </li>
                </ul>
        </section>

        <section class="clientes" id="secao3">

            <div class="titulo_restaurates">
                <h1>ALGUNS DE NOSSOS CLIENTES</h1>
            </div>
           <div class="clientes_restaurates">
                <img src="/assets/Imagens_dos_restaurantes/outback.png" class="restaurante_um">
                <img src="/assets/Imagens_dos_restaurantes/coco_bambu.png" class="restaurante_dois">
                <img src="/assets/Imagens_dos_restaurantes/fogo_de_chao.png" class="restaurante_tres">
                <img src="/assets/Imagens_dos_restaurantes/restaurante.png" class="restaurante_quatro">
            </div>

        </section>

        <section class="conteudo_cadastre" id="secao4">

            <div class="bloco_text">
                <h1>Cadastre o seu restaurante e profissionalize a forma com que gerencia suas reservas.</h1>
                <p>Cadastre o seu restaurante conosco e leve a gestão das suas reservas para um novo patamar de profissionalismo. Com nossa plataforma intuitiva e eficiente, você terá total controle sobre as reservas, podendo gerenciá-las de forma simples e organizada. Aumente a eficiência do seu estabelecimento e proporcione uma experiência ainda melhor para seus clientes, garantindo que cada reserva seja tratada com atenção e cuidado. Junte-se a nós e otimize a operação do seu restaurante hoje mesmo.</p>
            </div>

            <figure class="caixa">

                <img src="/assets/caixa.png" alt="">

            </figure>

        </section>

        <footer class="rodape">

            <div class="conteudo_rodape">

                <img src="/assets/logo.png" alt="">

            </div>

        </footer>
    </main>
</body>

</html>